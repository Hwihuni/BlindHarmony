...
def get_data(dataset, num_bits, train=True, valid_frac=None):
    ...
    if dataset == 'imagenet-64-fast':
        ...
        # root = dataset_root('ADNI')
        c, h, w = (1, 144, 176)

        if train:
            train_dataset = data.ADNI(
                path="/fast_storage/hwihun/pkls_BH/folder35177_train.pklv4",
                transform=Preprocess(num_bits)
            )
            valid_dataset = data.ADNI(
                path="/fast_storage/hwihun/pkls_BH/folder35177_val.pklv4",
                transform=Preprocess(num_bits)
            )
            '''
            num_train = len(train_dataset)
            ...
            # use validation data as test data & set a part of the train data as val data
            test_dataset = data.ADNI(
                path="/fast_storage/hwihun/pkls_BH/folder35177_test.pklv4",
                transform=Preprocess(num_bits)
            )
        ...
        if train:
            train_dataset = data.BRATS(
                path="/home/hwihun/blindharmony/dataset_tmp/pkls_brats/folder1_train_t1.pklv4",
                transform=Preprocess(num_bits)
            )
            valid_dataset = data.BRATS(
                path="/home/hwihun/blindharmony/dataset_tmp/pkls_brats/folder1_val_t1.pklv4",
                transform=Preprocess(num_bits)
            )
            '''
            num_train = len(train_dataset)
            ...
            # use validation data as test data & set a part of the train data as val data
            test_dataset = data.BRATS(
                path="/home/hwihun/blindharmony/dataset_tmp/pkls_brats/folder1_test_t1.pklv4",
                transform=Preprocess(num_bits)
            )
